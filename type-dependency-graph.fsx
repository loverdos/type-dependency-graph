...
#r @"packages/QuickGraph.3.6.61119.7/lib/net4/QuickGraph.dll"
...
type TypeWithDependencies = 
    { typeDef: TypeName; publicDeps: TypeDependency Set; implDeps: TypeDependency Set}
...
module AssemblyTypes = 
    ...
    /// Return true if the type was generated by the compiler
    /// These types will not be included in counts and diagrams.
    let isCompilerName (s:string) = 
        s.Contains("__")   // C#
        || s.Contains("<") // C#
        || s.Contains("@") // F# 
        || s.Contains("$") // F# 
    ...
    /// Get all top level types from an assembly
    let topLevelTypes excludeCore assemblyFileName = 
        let isNotSpecial (td:TypeDefinition) = not <| isCompilerName td.FullName
...
module CyclicDependencies = 
    ...
    /// return the total number of non-trivial components
    let cycleCount depSets = 

        // get all the components
        let components = scc depSets 

        components 
        |> Seq.filter (fun set -> set.Count > 1)
        |> Seq.length
...
module GraphViz = 
    ...
    let graphVizPath = @"/usr/local/bin/"
    ...
    let generateImageFile dotFilename algo format imgFilename =
        let cmd = sprintf "%s%s" graphVizPath algo 
...
module Stats = 
    ...
    let isAuthored (TypeName s) = AssemblyTypes.isCompilerName s |> not 
    ...
    let typeStats topLevelTypes = 
        ...
        let authoredTypeCount = 
            ...
            |> Seq.filter (fun td -> td.typeDef |> isAuthored) 
            |> Seq.length
        let allTypeCount = 
            topLevelTypes 
    ...
    let dependencyStats depSets = 
        ...
        let components = CyclicDependencies.scc depSets 
        let cycles = CyclicDependencies.toCyclicDependencies depSets
        let cycleCount = CyclicDependencies.cycleCount cycles 
        let maxComponentSize = CyclicDependencies.maxComponentSize components 
        ...
        { depCount=depCount; 
          ...
          cycleCount=cycleCount; maxComponentSize=maxComponentSize }
...
module Output = 
    ...
    let printStats projectName codeSize typeStats publicDepStats allDepStats = 
        ...
        printfn " - Cycle count=%i maxComponentSize=%i" allDepStats.cycleCount allDepStats.maxComponentSize
        ...
        printfn " - Cycle count=%i maxComponentSize=%i" publicDepStats.cycleCount publicDepStats.maxComponentSize
...
let tabularStats projects = 
    printfn "Project,CodeSize,TopLevelTypes,AuthoredTypes,AllTypes,AllTotalDepCount,AllOneOrMoreDepCount,AllThreeOrMoreDepCount,AllFiveOrMoreDepCount,AllTenOrMoreDepCount,AllMaxComponentSize,AllCycleCount,PubTotalDepCount,PubOneOrMoreDepCount,PubThreeOrMoreDepCount,PubFiveOrMoreDepCount,PubTenOrMoreDepCount,PubMaxComponentSize,PubCycleCount"
    for (projectName,assemblyName) in projects do
        ...
        let publicDepStats = Stats.dependencyStats publicDeps
        let allDepStats = Stats.dependencyStats allDeps
        printfn "%s,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i" projectName codeSize typeStats.topLevelTypeCount typeStats.authoredTypeCount typeStats.allTypeCount allDepStats.totalDepCount allDepStats.oneOrMoreDeps allDepStats.threeOrMoreDeps allDepStats.fiveOrMoreDeps allDepStats.tenOrMoreDeps allDepStats.maxComponentSize allDepStats.cycleCount publicDepStats.totalDepCount publicDepStats.oneOrMoreDeps publicDepStats.threeOrMoreDeps publicDepStats.fiveOrMoreDeps publicDepStats.tenOrMoreDeps publicDepStats.maxComponentSize publicDepStats.cycleCount 
...
// Install-Package Mono.Cecil -Version 0.9.5.4
let cecil = @"packages/Mono.Cecil.0.9.5.4/lib/net40/Mono.Cecil.dll"

// C# Project - NUnit.Framework
// Install-Package NUnit -Version 2.6.2
let nunit = @"packages/NUnit.2.6.2/lib/nunit.framework.dll"

// C# Project - Nancy
// Install-Package Nancy -Version 0.17.1
let nancy = @"packages/Nancy.0.17.1/lib/net40/Nancy.dll"
...
let specFlow = @"packages/SpecFlow.1.9.0/lib/net35/TechTalk.SpecFlow.dll"

// C# Project - SignalR
// Install-Package Microsoft.AspNet.SignalR.Core -Version 1.1.1
let signalR = @"packages/Microsoft.AspNet.SignalR.Core.1.1.1/lib/net40/Microsoft.AspNet.SignalR.Core.dll"

// C# Project - JSON.NET
// Install-Package Newtonsoft.Json -Version 5.0.5
let jsonDotNet = @"packages/Newtonsoft.Json.5.0.5/lib/net40/Newtonsoft.Json.dll"

// C# Project - EntityFramework 
// Install-Package EntityFramework -Version 5.0.0
let ef = @"packages/EntityFramework.5.0.0/lib/net40/EntityFramework.dll"
...
let nuget = @"packages/Nuget.Core.2.5.0/lib/net40-Client/NuGet.Core.dll"
...
let fsPowerPack = @"packages/FSPowerPack.Core.Community.3.0.0.0/lib/net40/Fsharp.PowerPack.dll"

// F# Project - FParsec
// Install-Package FParsec -Version 0.9.2.0
let fsParsec = @"packages/FParsec.0.9.2.0/lib/net40/FParsec.dll"
...
let fsYaml = @"packages/FsYaml.1.1.2/lib/net40/FsYaml.dll"
...
let tickSpec = @"packages/TickSpec.1.0.0.1/Lib/net40/TickSpec.dll"
...
analyzeAndGenerate "fsParsec" fsParsec 
